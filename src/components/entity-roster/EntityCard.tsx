...
export const EntityCard = memo(function EntityCard({
  ...
  const hpRatio = useMemo(
    () => (entity.maxHp.lte(0) ? 0 : entity.currentHp.dividedBy(entity.maxHp).toNumber()),
    [entity.currentHp, entity.maxHp]
  );
  const resourceRatio = useMemo(
    () =>
      entity.maxResource.lte(0)
        ? 0
        : entity.currentResource.dividedBy(entity.maxResource).toNumber(),
    [entity.currentResource, entity.maxResource]
  );
  const expRatio = useMemo(
    () => (entity.expToNext.gt(0) ? entity.exp.dividedBy(entity.expToNext).toNumber() : 0),
    [entity.exp, entity.expToNext]
  );
  const tooltipId = useMemo(() => `${entity.id}-stats-tooltip`, [entity.id]);

  // Memoize build profile calculation - this is expensive
  const buildProfile = useMemo(() => getHeroBuildProfile(entity, buildState), [entity, buildState]);

  // Filter combat events for this entity only
  const entityCombatEvents = useMemo(
    () =>
      combatEvents.filter((event) => event.sourceId === entity.id || event.targetId === entity.id),
    [combatEvents, entity.id]
  );
    ...
    >
      <div className="p-3 space-y-2.5">
        ...
        >
          ...
          <CombatEventStack entityId={entity.id} combatEvents={entityCombatEvents} />
          <EntityTooltip
            entity={entity}
            tooltipId={tooltipId}
