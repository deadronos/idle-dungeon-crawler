...
* **On-demand build and derived detail:** the roster hover/focus tooltip now carries the MVP combat ratings (`power`, `spellPower`, `precision`, `haste`, `guard`, `resolve`, `potency`, `crit`), the existing combat-facing derived detail (`ACC`, `EVA`, `PAR`, `APEN`, `EPEN`, `TEN`), plus passive, talent, equipment, resistance, and status summaries instead of permanently expanding every card.
