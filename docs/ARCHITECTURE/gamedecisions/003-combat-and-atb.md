...
### State Updates
To prevent excessive UI re-renders, the React `gameState` only commits state draft updates if an action was actually taken during that game tick, although ATB values continuously update in the background.
