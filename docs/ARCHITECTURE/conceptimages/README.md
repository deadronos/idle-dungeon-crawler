...
- `007-status-effects-view.png` — dungeon gameplay view with reusable status chips visible on roster cards, status-focused log entries, and tooltip status details expanded alongside existing combat stats. **Note:** as of the buff/debuff framework extension, status chips are now polarity-aware — buff chips (`RGN`) render in emerald green and debuff chips (`BRN`, `SLW`, `WKN`, `HEX`) render in red. The tooltip section header is neutral slate. This screenshot predates that change and should be recaptured when the app is running with a mixed buff/debuff party state visible.
